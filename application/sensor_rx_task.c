...
/**
  ...
  * @retval         none
  */
void sensor_rx_task(void const *pvParameters)
{
    //wait a time
    ...
    while (1)
    {
        ...
        // Read out raw data and send to PC
        // 读取原始数据并发送到PC
        INS_accel[INS_ACCEL_X_ADDRESS_OFFSET] = bmi088_real_data.accel[0];
        INS_accel[INS_ACCEL_Y_ADDRESS_OFFSET] = bmi088_real_data.accel[1];
        INS_accel[INS_ACCEL_Z_ADDRESS_OFFSET] = bmi088_real_data.accel[2];
        INS_gyro[INS_GYRO_X_ADDRESS_OFFSET] = bmi088_real_data.gyro[0];
        INS_gyro[INS_GYRO_Y_ADDRESS_OFFSET] = bmi088_real_data.gyro[1];
        INS_gyro[INS_GYRO_Z_ADDRESS_OFFSET] = bmi088_real_data.gyro[2];
        INS_mag[INS_MAG_X_ADDRESS_OFFSET] = ist8310_real_data.mag[0];
        INS_mag[INS_MAG_Y_ADDRESS_OFFSET] = ist8310_real_data.mag[1];
        INS_mag[INS_MAG_Z_ADDRESS_OFFSET] = ist8310_real_data.mag[2];
    }
}
